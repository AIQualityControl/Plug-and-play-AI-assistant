...
def get_seg_model(cfg, **kwargs):
    if cfg.MODEL.NET == 'Unet':
        model = U_Net(out_ch=cfg.DATASET.NUM_CLASSES)
    elif cfg.MODEL.NET == 'R2Unet':
        model = R2U_Net(output_ch=cfg.DATASET.NUM_CLASSES)
    elif cfg.MODEL.NET == 'Att_Unet':
        model = AttU_Net(output_ch=cfg.DATASET.NUM_CLASSES)
    elif cfg.MODEL.NET == 'R2Att_Unet':
        model = R2AttU_Net(out_ch=cfg.DATASET.NUM_CLASSES)
    elif cfg.MODEL.NET == 'NestedUnet':
        model = NestedUNet(out_ch=cfg.DATASET.NUM_CLASSES)
    else:
        print(f'{cfg.MODEL.NET} is not supported yet!')
        return None
...
def build_model(checkpoint_path, pretrained=False, model_name='Unet'):
    if model_name == 'Unet':
        model = U_Net(out_ch=2)
    elif model_name == 'R2Unet':
        model = R2U_Net(output_ch=2)
    elif model_name == 'Att_Unet':
        model = AttU_Net(output_ch=2)
    elif model_name == 'R2Att_Unet':
        model = R2AttU_Net(out_ch=2)

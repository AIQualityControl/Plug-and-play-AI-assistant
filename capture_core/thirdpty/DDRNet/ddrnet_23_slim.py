...
class DualResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, planes=64, spp_planes=128, head_planes=128, augment=True):
    ...
    def forward(self, x):
        ...
        x_ = F.interpolate(x_, size=[width_output, height_output], mode='bilinear')

        if self.augment:
            x_extra = self.seghead_extra(temp)
            x_extra = F.interpolate(x_extra, size=[width_output, height_output], mode='bilinear')
            return [x_, x_extra]
        else:
            return x_

...
class HarDMSEG(nn.Module):
    ...
    def __init__(self, channel=32, checkpoint_path='', pretrained=True):
        ...
        self.hardnet = hardnet(arch=68, pretrained=pretrained, checkpoint_path=checkpoint_path)
...
def build_model(checkpoint_path, pretrained=False):
    model = HarDMSEG(pretrained=pretrained, checkpoint_path=checkpoint_path)
    if pretrained:
...
def get_seg_model(cfg, **kwargs):
    model = HarDMSEG(checkpoint_path=cfg.MODEL.PRETRAINED, pretrained=True)
    return model


# if __name__ == '__main__':
#     ras = HarDMSEG(pretrained=False).cuda()

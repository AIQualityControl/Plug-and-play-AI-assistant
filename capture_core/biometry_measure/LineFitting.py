...
class LineFitting:
    ...
    def fit_line(cls, gray_image, mask):
        ...
        dll = int(bbox[2] * 0.06)  # drop 0.06 position on both sides to avoid fitting on boundary
        for i in range(bbox[0] + dll, bbox[0] + int(bbox[2] * 0.2), 3):  # get 2 endpoint 0.2 points for line fitting

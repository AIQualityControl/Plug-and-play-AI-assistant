...
class LVMeasure:
    ...
    def do_measure(cls, type2mask, image, detection_info, measure_mode='hadlock', measure_hc_bpd=True, qn_flag=False):
        ...
        if "posterior horn of the lateral ventricle" not in type2mask:
            return hc_info, bpd_infos, None
        ...
        cns_contours = measure_util.match_detection_info(type2mask, detection_info, "posterior horn of the lateral ventricle", smooth_ksize=15)
        if not cns_contours:
            return hc_info, bpd_infos, None
        cns_contour = cns_contours[0]

        mlc_contour = None
        if "choroid plexus" in type2mask:
            mlc_contours = measure_util.match_detection_info(type2mask, detection_info, "choroid plexus", smooth_ksize=15)
    ...
    def hc_and_bpd(cls, type2mask, image, measure_mode='hadlock'):
        #
        if "skull Halo" not in type2mask or len(type2mask["skull Halo"]) != 1:
            return None, None

        mask = type2mask["skull Halo"][0]['mask']
        ...
        if mlc_contour is None:
            ...
            if (abs(cnsMax_x[0] - mlcMax_x[0]) > abs(cnsMin_x[0] - mlcMin_x[0])):
                mlc_point = [mlcMax_x[0] + key_point_offset_ratio * (cnsMax_x[0] - mlcMax_x[0]),
                             mlcMax_x[1] + key_point_offset_ratio * (cnsMax_x[1] - mlcMax_x[1])]  
            else:
                mlc_point = [mlcMin_x[0] + key_point_offset_ratio * (cnsMin_x[0] - mlcMin_x[0]),
                             mlcMin_x[1] + key_point_offset_ratio * (cnsMin_x[1] - mlcMin_x[1])]  

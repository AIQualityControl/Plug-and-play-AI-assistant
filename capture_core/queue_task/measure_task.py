...
class MeasureTask(Task):
    ...
    def init_models(self, model_params, gpu_id, load_model):
        ...
        for image_type_name, model_param in model_params.items():
            if not self.config['measure_heart_spine'] and image_type_name in ('four-chamber heart measurement plane', 'spine measurement plane'):
                continue
            # set measure mode
    ...
    def construct_name2id_map(self):
        ...
        self.AMNIOTIC_PLANE_ID = self.id_of_image_type('amniotic fluid measurement plane')
    ...
    def process_thyroid_image(self, image_info):
        auto_type = self.id_of_image_type('thyroid plane')
        model = self.get_model(image_info, auto_type)
        if model is None:
            logger.info(f'frame_idx: {image_info.frame_idx}, auto type: {auto_type}')
            raise Exception('Failed to load segmentation model for thyroid plane')
    ...
    def measure_biometry(self, image_info, plane_type=None):
        ...
        elif image_info.capture_type == SPECTURM_MEASURE_MODE or plane_type == 'spectral measurement plane':
        ...
        if image_info.anno_set is None:
            ...
            # used for debug
            if image_info.capture_type == AMNIOTIC_MEASURE_MODE:
                image_type = 'amniotic fluid measurement plane'
            else:
    ...
    def test_for_measure(self, image_info, cur_image_info, sm_config: Config = None):
        ...
        if 'seg_roi' in image_info.detection_results:
            ...
            result = None
            if image_type == 'femur long-axis plane':
                result = FLMeasureInfo(pt_start, pt_end, ruler_unit)
            elif image_type == 'humerus long-axis plane':
                result = HLMeasureInfo(pt_start, pt_end, ruler_unit)
            elif image_type == "Horizontal cross-section of upper abdomen":
                result = AcMeasureInfo(pt_start, pt_end, 45, ruler_unit)
            elif image_type == 'thalamic transverse plane':
...
if __name__ == '__main__':
    ...
    for file_name in os.listdir(IMAGE_DIR):
        ...
        image_info.class_results = [{
            'class_type': task.id_of_image_type('femur long-axis plane'),
            'class_score': 1.0
        }]

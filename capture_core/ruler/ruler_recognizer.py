...
class SingleRulerInfo(ctypes.Structure):
    _fields_ = [("rulerUnit", ctypes.c_double),
                ("startX", ctypes.c_int),
                ("startY", ctypes.c_int),
                ("endX", ctypes.c_int),
                ("endY", ctypes.c_int),
...
class RulerRecognizer:
    ...
    try:
        ...
        detect_ruler_func = rulerDll.detect_ruler_for_quality_control
    ...
    def detect_ruler(cls, origin_image, roi=None, sm_config: Config = None) -> SingleRulerInfo:
        ...
        char_box = (ctypes.c_int * 4)(0, 0, 0, 0)
        try:
            # ruler_info = RulerRecognizer.detect_ruler_func(dataptr, width, height, channels, roi_c)
            ruler_info = RulerRecognizer.detect_ruler_func(dataptr, width, height, channels, char_box, roi_c)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    import cv2
    image_path = r"femur_images/Testing_femur1.jpg"

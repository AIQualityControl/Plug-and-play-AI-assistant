...
class QcModel:
        ...
        if load_model:
            ...
            if not os.path.exists(model_path):
                ...
                if os.path.exists(abs_path):
                    ...
                    logger.error(f'Model file does not exist: {model_path} or {abs_path}')
                    return False
    ...
    def get_std_info(cls, part):
        ...
        if len(items) < 2:
            items = part.split('-')
        if len(items) < 2:
            if part.endswith('nonstandard'):
                std_info = 'nstd'
                part = part[:-3]
            elif part.endswith('NSTD') or part.endswith('nstd'):
                std_info = 'nstd'
                part = part[:-4]
            elif part.endswith('standard'):
                std_info = 'std'
                part = part[:-2]
            ...
            if items[1] in ['nonstandard', 'nstd', 'NSTD']:
                std_info = 'nstd'
            elif items[1] in ['standard', 'std', 'STD']:
                std_info = 'std'
    ...
    def normalize_conf(score, pass_score):
        """
        standard score: [60, 100], non-standard score: [0, 60)
        """
        norm_score = 0
        if score < 0:
            norm_score = 0

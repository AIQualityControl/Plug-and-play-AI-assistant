...
class BaseTest():
    ...
    def run(self, start_frame_idx=0, save_result=False, anno_file='annotations.json'):
        ...
        if self.test_with_annotations:
            # load annotations judge if annotation exists
            if not self.anno_path:
                logger.error('annotation file does not exist')
            else:
                try:
                # get the names and corresponding annotations for each image
                    self.anno_parser = AnnotationConverter(self.anno_path, polygon_to_box=False)
            
                    if hasattr(self.anno_parser, 'config') and self.anno_parser.config:
        ...
        if self.image_name:
            ...
            result = self.test_image(image, anno_set, 0, self.image_name)
            
            if result:
                self.image2anno[self.image_name] = result
            cv2.waitKey(0) 
        ...
        # process videos
            ...
            while True:
                ...
                key = cv2.waitKey(5 if not is_paused else 0) & 0xFF
                if key == 27:  # ESC 
                    break
                if key == 32:  # space
                ...
                if is_paused:
                    continue  # stay on the current frame when paused

                anno_set = self.get_annotation_set(str(frame_idx))
                try:
                    if anno_set.plane_type == 'femur long-axis plane':
            ...
            if save_result:
                ...
                anno_path = os.path.join(video_path, video_name + '_result.json')
                self.save_result(anno_path)
                # anno_path = os.path.join(video_path, anno_file)
                # self.save_result(anno_path)
